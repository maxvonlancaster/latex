...
This is a binary distribution for Windows. You can find the original
sources used to compile it on the Ipe homepage (see below).

In addition to this package, you also need
"ipe-7.0.0-dependencies-win.zip", which contains DLLs of the libraries
that Ipe depends on.  These dependencies are distributed separately
because they are large, and change only rarely.
...
characters.  Unzip "ipe-7.0.0-dependencies-win.zip" into the top ipe
directory (the one containing this readme file).  You can then find
(and start) the Ipe executable in the "bin" subdirectory.
