...
\newcommand*{\dyddiad}{17\textsu{th} September, 2010}
\date{\dyddiad}
\pagestyle{fancy}
...
\begin{longtable}{llp{.2\textwidth}p{.175\textwidth}}
	...
	\textbf{encoding file}	&	\textbf{provides}	&	\textbf{figures}		&	\textbf{empty slots $\rightarrow$}	\\\midrule\endhead
		\bottomrule\endfoot
		texnansi-ybd.enc			&	TeXnANSIEncoding-ybd			&	tabular lining					&	\multirow{4}{0.175\textwidth}{fj, ffj, ft, fft, tt}\\
		...
		texnansi-ybd1.enc		&	TeXnANSIEncoding-ybd1		&	\multicolumn{2}{c}{superiors, as available}\\
\end{longtable}
...
\subsubsection{Styles}
	...
	\begin{itemize}
		...
		\item \verb|fj| \verb|ffj| \verb|ft| \verb|fft| \verb|tt| will produce ligatures (\textswash{fj/ffj/ft/fft/tt});
		\item \verb|ch| \verb|ck| \verb|cl| \verb|ct| \verb|sh| \verb|sk| \verb|sl| \verb|sp| \verb|st| will produce additional ligatures (\textswash{ch/ck/cl/ct/sh/sk/sl/sp/st});
	...
	\begin{itemize}
		...
		\item \verb|fj| \verb|ffj| \verb|ft| \verb|fft| \verb|tt| will produce ligatures (fj/ffj/ft/fft/tt);
		\item \verb|ch| \verb|ck| \verb|cl| \verb|ct| \verb|sh| \verb|sk| \verb|sl| \verb|sp| \verb|st| will not produce ligatures (ch/ck/cl/ct/sh/sk/sl/sp/st);
		\item \verb|\zeroslash| will not produce the slashed zero even if lining figures are in use (\textl{\zeroslash/\texttl{\zeroslash}});
		\item \verb|\^W|, \verb|\^w|, \verb|\^Y| and \verb|\^y| will create accented characters built from separate glyphs for the accent and base letter (\^W/\^w/\^Y/\^y).
	\end{itemize}	
...
The final two rows list commands for accessing superiors and inferiors.
	...
	In addition to modifying the figure style, these commands affect the style of certain complementary characters in the \textsc{ly1} as explained in \fref{sec:encs}. This means that:
		\begin{verbatim}
