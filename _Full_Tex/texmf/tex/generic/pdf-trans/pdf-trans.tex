...
% Public Domain
%
% The macro provides a bunch of TeX box transformations. It was initially inspired by
% trans.tex (BOP, bop@bop.com.pl), remade to work with pdfeTeX. This standalone code
% gathers parts of pdfeplay macro package.
%
% Files:
%     pdf-trans.tex
%     example.tex
...
%     15.2005, v2.1
%         * some adds;
%             - \transround <0..4> sets pdf dimen rounding precision
...
% Each transformation macro defined below expands to (h)box. Thus, can be used whenever
% (h)box can. Each must be followed by the box that is to be transformed. This scheme
% allows to cumulate transformations, that are applied from right to left. For instance,
%     \boxrotatebb{30}\boxscale{300}\hbox{Aqq}
% first enlarges  boxed `Aqq' three times, then rotates it 30 degrees. Thanks to \transboxcheck
% trick (see below), \copy and \box can be used as well as \hbox, \vbox, \vtop. However,
% the consequence is that the transformed box can't be void (\boxabout definition is an
% exception here). \transbox register contains the box being transformed.

\newbox\transbox

% Whenever we perform relative dimension scaling, a number provided as a macro parameter is
% divided by \transfactor. Default is percentage (ie. 75 -> .75). One may notice, that we
% often perform the same operation (ie. division) several times instead of defining
% \transfactor as a float-like string. In example, if there are two dimens, \X and \Y, and
% both are to be scaled by 0.23, we always say `\numexpr\X*23/\transfactor' and
% `\numexpr\Y*23/\transfactor' instead of 0.23\X 0.23\Y. In some cases it gives significant
...
% Here the main trick starts. Each macro usually expands to \hbox\bgroup..., defines
% \transboxtodo procedure, assigns the following box, then performs \transboxtodo and ends
% with \egroup. If the box being transformed is generic `\hbox{', TeX finishes assignment
% just after opening `{', while the box being assigned (\transbox) is still void. Thus, we
% check \ifvoid\transbox, and if so, we put \transboxtodo on \aftergroup stack to let TeX
% finish box assignment. Obviously it crashes, if one want to transform void box.
...
% When some definition silently changes box dimensions (i.e. \ht\transbox=0pt),
...
% User-defined operation on \transbox. Remember, that \transboxcheck assumes initially void \transbox,
% so #1 should contain something like \box\tranbox, \unvbox\tranbox, \setbox\transbox\voidb@x etc.

\long\def\hboxtrans#1{%
 \hbox\bgroup
  \def\transboxtodo{#1\egroup}\transhboxini}

\long\def\vboxtrans#1{%
 \vbox\bgroup
  \def\transboxtodo{#1\egroup}\transhboxini}

\long\def\vtoptrans#1{%
 \vtop\bgroup
  \def\transboxtodo{#1\egroup}\transhboxini}
...
\def\boxflipy{%
 \hbox\bgroup
  \def\transboxtodo{%
   \pdfliteral{q 1 0 0 -1 0 \tobp{\ht\transbox-\dp\transbox}\space cm}%
   \savebp\trans:def\wd\transbox
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
   \egroup}\transboxini}

%\def\boxflipxy{\boxflipx\boxflipy}

\def\boxflipxy{%
 \hbox\bgroup
  \def\transboxtodo{%
   \savebp\trans:def\wd\transbox
   \pdfliteral{q -1 0 0 -1 \trans:def\space \tobp{\ht\transbox-\dp\transbox}\space cm}%
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
   \egroup}\transboxini}
...
\def\boxtranslate#1#2{%
 \hbox\bgroup
  \def\transboxtodo{%
   \pdfliteral{q 1 0 0 1 \tobp{#1}\space \tobp{#2}\space cm}%
   \savebp\trans:def\wd\transbox
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
  \egroup}\transboxini}
...
\def\boxrevolveleft{%
 \hbox\bgroup
  \def\transboxtodo{%
   ...
   \dp\transbox=\z@
   \pdfliteral{q 0 1 -1 0 \tobp\trans:dim:b\space 0 cm}%
   \savebp\trans:def\wd\transbox
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
   \egroup}\transhboxini}
...
\def\boxrevolveright{%
 \hbox\bgroup
  \def\transboxtodo{%
   ...
   \ht\transbox=\trans:dim:a
   \dp\transbox=\z@
   \pdfliteral{q 0 -1 1 0 \tobp\trans:dim:b\space \tobp{\ht\transbox}\space cm}%
   \savebp\trans:def\wd\transbox
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
   \egroup}\transhboxini}
...
\let\boxrotatebb\boxrotatebbl

% slanting; note, that \boxslantx{<x>}\boxslanty{<y>} is NOT equivalent to \boxslantxy{<x>}{<y>}
...
\let\boxslantbb\boxslantbbl
\def\boxslantbbx#1{\boxslantbb{#1}\z@}
\def\boxslantbbry#1{\boxslantbbr\z@{#1}}
\def\boxslantbbly#1{\boxslantbbl\z@{#1}}
\let\boxslantbby\boxslantbbly
...
% The previous version of pdftrans had \boxclip defined as \boxpath{}{W n}. This didn't
...
\def\boxclip{%
 \hbox\bgroup
  \def\transboxtodo{%
   \savebp\trans:def\wd\transbox
   \pdfliteral{q 0 \tobp{-\dp\transbox}\space \trans:def\space
               \tobp{\ht\transbox+\dp\transbox}\space re W n}%
   \box\transbox
   \pdfliteral{Q 1 0 0 1 \trans:def\space 0 cm}%
   \egroup}\transboxini}
...
% box with rule-like spec of the box size (risky hack)
...
% reboxing; restore natural dimensions (without shrink or stretch).

\def\revbox{%
 \vbox\bgroup
  \def\transboxtodo{\unvbox\transbox\egroup}\transboxini}
...
\def\rehbox{%
 \hbox\bgroup
  \def\transboxtodo{\unhbox\transbox\egroup}\transboxini}

% resizing; the result may be different for hboxes and vboxes

\def\boxresizeto#1#2#3{%
 \hbox\bgroup
  \def\transboxtodo{% empty parameter means no change
...
% extend; side effect is that \dp\transbox becomes 0pt
...
\def\hboxwrap{%
 \hbox\bgroup
  \def\transboxtodo{\box\transbox\egroup}%
 \transboxini}
...
\def\box:@bout#1{%
 ...
 \ifx#1\trans:def:a\errormessage{Not a box #1}\fi\let\trans:def:a#1% endless loop otherwise
...
% Whenever we write a dimen into PDF code, we need to convert it from TeX units to
% Postscript big points. We handle that as precise as possible, using the fact that eTeX
% handles 64bit numbers as temporary results of expressions such as A*B/C. A is dimension
% in points, B is \pt:f@ctor and C is \bp:f@ctor. Note that factor 100pt/100bp is more
% precise than 1pt/1bp or 10pt/10bp, but NOT less precise than 1000pt/1000bp. Thus, 100 is
% the optimum.
...
% If we assume that \pdfdecimaldigits never get values out of possible range (0..4),
...
\def\roundbpoff{\let\tobp\t@bp}
\def\roundbpon{\let\tobp\roundbp}

%\roundbpoff
\roundbpon

% Lets play with basic arithmetic operations. To make things consistent, each function
% expands to \numexpr|\dimexpr, even if could be easily expanded to digits. This approach
% ensures predictable behaviour whenever a \function is followed by \expandafter or \relax.
% To avoid evaluating the same expressions twice or more, each function expands its
% parameters before performing the final operation. This scheme makes temporary macros
% pretty reusable in other functions.
...
% Possible approaches to integer division:
...
% One sticky problem in all the division related macros above... We always check
% if some rounding is present using the formula
%     \divident/\divisor*\divisor <=> \divident
% It works fine if one say (2\maxdimen)/\maxdimen, but arithmetic overflow occurs
% for (2\maxdimen)/(\maxdimen+1) or (2\maxdimen)/2. Thus, can be used for reasonably small
% numbers.

% Having integer division we can round float-like strings
...
% purposes, but floats are limited to \maxdimen expressed in points
% (16383.99998).
...
% however, internally we always use \dimexpr\hsize*2/7 in such cases
% (see pdftrans.tex).

% There is another approach to division with float-like result, this time with fixed
% preciseness. Here we also have numeric overflow risk mentioned above.
...
% And now comes a real challenge -- trigonometry. In the first approach, values of
% trigonometric functions were predefined for angles of range 0..90. Real angle values were
% not supported. Now things are a bit slower, but much more precise. In particular, real
% angles values are now supported. This approach is excerpted from trans.tex and originally
% was inspired by mf.web. It is worthy to note that any angle value (not only integer) can
% be represented as a series of predefined ,,coins''. Thus, trigonometric functions can be
% precisely calculated as follows:
%
...
% scaled sine and cosine. This makes things flexible and reusable.
...
% In pdftrans macro we use the following shortcuts
